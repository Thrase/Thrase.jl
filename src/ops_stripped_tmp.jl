...
function get_operators(p, Nx, Nz, μ, Lx, Lz)
...
# Derivative operators for the rest of the computation
    (Dx, HxI, Hx, x) = diagonal_sbp_D1(p, Nx; xc = (0,Lx))
    Qx = Hx * Dx
    QxT = sparse(transpose(Qx))

    (Dz, HzI, Hz, z) = diagonal_sbp_D1(p, Nz; xc = (0,Lz))
    Qz = Hz * Dz
    ...
    Ix = sparse(I, Nxp, Nxp)
    Iz = sparse(I, Nzp, Nzp)
    (_, S0x, SNx, _, _, Ax, _) = variable_diagonal_sbp_D2(p, Nx, μ * ones(Nxp); xc = (0,Lx))
    (_, S0z, SNz, _, _, Az, _) = variable_diagonal_sbp_D2(p, Nz, μ * ones(Nzp); xc = (0,Lz))
