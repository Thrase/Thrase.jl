...
function get_operators_BP6_trac(p, Nr, Ns, μ, Lx, Lz; metrics=create_metrics(p,Nr,Ns))
...
Ã = Ãrr + Ãss + Ãrs + Ãsr
  ...
  # Modify operators for the BC (Neumann only only faces 2, 3 and 4)
  M̃ -= F2 * (Diagonal(1 ./ (diag(τ2))) * H2I) * F2'
  M̃ -= F3 * (Diagonal(1 ./ (diag(τ3))) * H3I) * F3'
  M̃ -= F4 * (Diagonal(1 ./ (diag(τ4))) * H4I) * F4'
...
function bdry_vec_mod_BP6_trac!(g, F, τ, r, s, bc_Dirichlet, bc_Neumann, metrics)
  Nr = length(r)
  Ns = length(s)
  ...
  # FACE 2 (Neumann) (right):
  lf = 2
  gN = bc_Neumann(lf, xf[lf], yf[lf], 1, 0)
  vf = gN  ./ diag(τ[lf])
  g[:] -= F[lf] * vf
  ...
  # FACE 3 (Neaumann) (top):
  lf = 3
  gN = bc_Neumann(lf, xf[lf], yf[lf], 0, -1)
  vf = gN  ./ diag(τ[lf])
  g[:] -= F[lf] * vf

  # FACE 4 (Neaumann) (bottom):
  lf = 4
  gN = bc_Neumann(lf, xf[lf], yf[lf], 0, 1)
  vf = gN  ./ diag(τ[lf])
  g[:] -= F[lf] * vf
...
function computetraction_stripped_BP6(HfI_FT, sJ, τ, lf, u, δ)
  ...
  export get_operators_BP6_trac,  bdry_vec_mod_BP6_trac!