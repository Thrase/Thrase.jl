...
EToDomain = [1 1 2 2]
    ...
    # Plot the grid
    if do_plotting
        p = Plots.scatter(verts[1,:], verts[2,:], markersize=2, markercolor=:blue, legend = false, size=(1600, 600))
        for e = 1:nelems
            V = EToV[:, e]
            V[3], V[4] = V[4], V[3]
            push!(V, V[1])
            Plots.plot!(verts[1, V], verts[2, V],linecolor=:red, legend = false, size=(1600, 600))
        end
        display(p)
    end
    ...
    NTYPE = typeof([l[11][1], l[11][2], l[11][3], l[11][4]])
    ...
    # initialize backgrounds stresses and changes in stress due to quasi-static deformation:
    σ11_0 = zeros(δNp)
    σ22_0 = zeros(δNp)
    σ33_0 = zeros(δNp)
    Δτ = zeros(δNp)
    ...
    cb = SavingCallback((ψδ, t, i)->savedatafields(ψδ, t, i, stations, fault, V_0, 
                                                    FToδstarts, odeparam,
                                                    pth*"BP9_N_$(Nr[1])_", pth*"Slip_BP9_N_$(Nr[1])_",  10year_seconds),
