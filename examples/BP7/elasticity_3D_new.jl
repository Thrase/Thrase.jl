...
# T_12_1_new = - (K_v - 2/3 * μ_v) * p_pz_new #* u1_filter # Not quite sure 
# T_13_1_new = - (K_v - 2/3 * μ_v) * p_py_new #* u1_filter

# T_21_1_new = μ_v * p_px_hat_new #* u2_filter
# T_22_1_new = - μ_v * p_py_new #* u2_filter
...
# T_12_2_new = (K_v - 2/3 * μ_v) * p_pz_new #* u1_filter
# T_13_2_new = (K_v - 2/3 * μ_v) * p_py_new #* u1_filter

# T_21_2_new = μ_v * p_px_hat_new #* u2_filter
...
# SAT_tilde_1_RHS_new =  (
#         (T_11_1_new' .- Z_11_1_new') * (HI_1 * e_1 * g₁¹[:])
#     +   (T_21_1_new' .- Z_21_1_new') * (HI_1 * e_1 * g₂¹[:])
#     +   (T_31_1_new' .- Z_31_1_new') * (HI_1 * e_1 * g₃¹[:])
#     +   (T_11_2_new' .- Z_11_2_new') * (HI_2 * e_2 * g₁²[:])
#     +   (T_21_2_new' .- Z_21_2_new') * (HI_2 * e_2 * g₂²[:])
#     +   (T_31_2_new' .- Z_31_2_new') * (HI_2 * e_2 * g₃²[:])
# )

# SAT_tilde_2_RHS_new =  (
#         (T_12_1_new' .- Z_12_1_new') * (HI_1 * e_1 * g₁¹[:])
#     +   (T_22_1_new' .- Z_22_1_new') * (HI_1 * e_1 * g₂¹[:])
#     +   (T_32_1_new' .- Z_32_1_new') * (HI_1 * e_1 * g₃¹[:])
#     +   (T_12_2_new' .- Z_12_2_new') * (HI_2 * e_2 * g₁²[:])
#     +   (T_22_2_new' .- Z_22_2_new') * (HI_2 * e_2 * g₂²[:])
#     +   (T_32_2_new' .- Z_31_2_new') * (HI_2 * e_2 * g₃²[:])
# )

# SAT_tilde_3_RHS_new =  (
#         (T_13_1_new' .- Z_13_1_new') * (HI_1 * e_1 * g₁¹[:])
#     +   (T_23_1_new' .- Z_23_1_new') * (HI_1 * e_1 * g₂¹[:])
#     +   (T_33_1_new' .- Z_33_1_new') * (HI_1 * e_1 * g₃¹[:])
#     +   (T_13_2_new' .- Z_13_2_new') * (HI_2 * e_2 * g₁²[:])
#     +   (T_23_2_new' .- Z_23_2_new') * (HI_2 * e_2 * g₂²[:])
#     +   (T_33_2_new' .- Z_33_2_new') * (HI_2 * e_2 * g₃²[:])
# )
...
u_direct_new = M_new \ RHS_new

error_direct_new = sqrt((u1_filter * u_direct_new - u1)' * H_tilde * (u1_filter * u_direct_new - u1) 
            + (u2_filter * u_direct_new - u2)' * H_tilde * (u2_filter * u_direct_new - u2) 
            + (u3_filter * u_direct_new - u3)' * H_tilde * (u3_filter * u_direct_new - u3) 
)

reshape((u1_filter * u_direct_new - u1), Nx, Ny, Nz)
reshape((u2_filter * u_direct_new - u2), Nx, Ny, Nz)
reshape((u3_filter * u_direct_new - u3), Nx, Ny, Nz)

sqrt((u1_filter * u_direct_new - u1)' * H_tilde * (u1_filter * u_direct_new - u1))
sqrt((u2_filter * u_direct_new - u2)' * H_tilde * (u2_filter * u_direct_new - u2))
sqrt((u3_filter * u_direct_new - u3)' * H_tilde * (u3_filter * u_direct_new - u3))


M_new_lu = lu(M_new)

@benchmark M_new_lu \ RHS_new

@benchmark cg(M_new, RHS_new)

M_new_gpu = CUDA.CUSPARSE.CuSparseMatrixCSR(M_new)
RHS_new_gpu = CuArray(RHS_new)

cg(M_new_gpu, RHS_new_gpu)

@benchmark cg(M_new_gpu, RHS_new_gpu)